...
function answerQuestion(answerIndex) {
    ...
    if (shuffledAnswers[answerIndex].isCorrect) {
        ...
        showFlashEffect();  // Эффект вспышки
    } else {
        lives--;
...
function createFallingObjects() {
    if (currentObjects < maxObjects) {
        ...
        object.src = "img/dollar.png";  // Можно использовать разные картинки для бонусов и долларов
...
function showFlashEffect() {
    ...
    setTimeout(() => flash.remove(), 500); // Эффект вспышки длится 0.5 секунд
}
